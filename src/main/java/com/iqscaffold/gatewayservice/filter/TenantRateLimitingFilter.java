...
public class TenantRateLimitingFilter implements GlobalFilter, Ordered {
  ...
  public TenantRateLimitingFilter(final IqScaffoldProperties properties,
  ...
  public Mono<Void> filter(ServerWebExchange exchange, GatewayFilterChain chain) {
    if (!properties.gateway().rateLimiting().enabled()) {
    ...
    return checkRateLimit(globalKey, rateLimitPolicy.requestsPerMinute(), rateLimitPolicy.burstCapacity())
        .flatMap(globalAllowed -> {
          ...
          if (rateLimitPolicy.enableTenantQuotas() && properties.gateway().rateLimiting().tenantQuotas().enabled()) {
  ...
  private IqScaffoldProperties.GatewayProperties.RateLimitingProperties.PoliciesProperties.EndpointPolicyProperties determineRateLimitPolicy(String path) {
    var policies = properties.gateway().rateLimiting().policies();
    ...
    return new IqScaffoldProperties.GatewayProperties.RateLimitingProperties.PoliciesProperties.EndpointPolicyProperties(
  ...
  private String createGlobalRateLimitKey(String clientIp, String path) {
    var keyPrefix = properties.gateway().rateLimiting().redis().keyPrefix();
  ...
  private int getTenantQuota(String tenantId) {
    var tenantQuotas = properties.gateway().rateLimiting().tenantQuotas();
